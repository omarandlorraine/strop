//! Implements peephole optimizations for 6502
...
fn check<I: Mos6502Compatibility>(
    cand: &Candidate<I>,
    a: u8,
    b: &[u8],
) -> Option<(usize, SearchCull<I>)> {
    let vec = &cand.instructions;
    for i in 0..vec.len() - 1 {
        if vec[i].encode()[0] == a && b.contains(&vec[i + 1].encode()[0]) {
...
fn commutative<I: Mos6502Compatibility>(
...
fn dead_code<I: Mos6502Compatibility>(candidate: &Candidate<I>) -> Option<(usize, SearchCull<I>)> {
    ...
    let destroy_carry = [
        // asl
        0x0a, 0x06, 0x16, 0x0e, 0x1e, // cmp
        0xc9, 0xc5, 0xd5, 0xcd, 0xdd, 0xd9, 0xc1, 0xd1, // cpx
        0xe0, 0xe4, 0xec, // cpy
        0xc0, 0xc4, 0xcc, // clc
        0x18, // sec
        0x38, // lsr
    ...
    let destroy_accumulator = [
        // lda
        0xa9, 0xa5, 0xb5, 0xad, 0xbd, 0xb9, 0xa1, 0xb1, // pha
        0x48, // txa
        0x8a, // tya
        0x98,
    ];
...
impl Peephole for Nmos6502Instruction {
    fn peephole(candidate: &Candidate<Self>) -> (usize, SearchCull<Self>) {
        if let Some(res) = dead_code(candidate) {
            res
        } else if let Some(res) = commutative(candidate) {
            res
        } else {
            (0, SearchCull::Okay)
        }
...
mod test {
    ...
    fn t() {
        ...
        let unoptimized_count = Nmos6502Instruction::bruteforce_search()
            .limit_length(2)
            .iter()
            .count();
        let optimized_count = Nmos6502Instruction::bruteforce_search()
            .peephole()
            .limit_length(2)
            .iter()
            .count();
        println!(
            "There are {} 6502 programs of length 1 or 2",
            unoptimized_count
        );
        println!(
            "The peephole optimizer removed {} from consideration",
