...
const ACC_OPCODES: [u8; 4] = [0x4a, 0x6a, 0x2a, 0x0a];

const IMP_OPCODES: [u8; 25] = [
...
const IMM_OPCODES: [u8; 11] = [
    0xa2, 0xe9, 0xc9, 0x09, 0x49, 0x29, 0xe0, 0xa9, 0xc0, 0xa0, 0x69,
];

const ZP_OPCODES: [u8; 21] = [
    0x86, 0x24, 0xc6, 0x84, 0x66, 0xe5, 0xa6, 0x85, 0xa5, 0x65, 0x05, 0xc4, 0xe6, 0x06, 0x26, 0xa4,
    0x25, 0xe4, 0x46, 0xc5, 0x45,
];

const ZPX_OPCODES: [u8; 16] = [
    0xd6, 0xb4, 0x35, 0xd5, 0xb5, 0x76, 0x36, 0x95, 0xf6, 0x94, 0xf5, 0x16, 0x55, 0x75, 0x15, 0x56,
];

const ZPY_OPCODES: [u8; 2] = [0x96, 0xb6];

const ABS_OPCODES: [u8; 23] = [
    0x20, 0x8d, 0xed, 0x4e, 0xcd, 0xae, 0x2e, 0x6e, 0xce, 0xac, 0x8c, 0xcc, 0xec, 0x2c, 0x4c, 0x4d,
    0x2d, 0x0e, 0xee, 0x6d, 0x8e, 0x0d, 0xad,
];

const ABSX_OPCODES: [u8; 15] = [
    0x9d, 0xfe, 0x7e, 0x3e, 0x5d, 0xfd, 0x5e, 0x7d, 0xde, 0x3d, 0x1e, 0xbd, 0x1d, 0xdd, 0xbc,
];

const ABSY_OPCODES: [u8; 9] = [0xbe, 0x79, 0xf9, 0xb9, 0xd9, 0x39, 0x99, 0x19, 0x59];

const IND_OPCODES: [u8; 1] = [0x6c];

const INDX_OPCODES: [u8; 8] = [0x81, 0x41, 0xc1, 0x21, 0xe1, 0x61, 0xa1, 0x01];

const INDY_OPCODES: [u8; 8] = [0xd1, 0x11, 0xb1, 0x51, 0xf1, 0x31, 0x91, 0x71];

const REL_OPCODES: [u8; 8] = [0x70, 0x50, 0xf0, 0x30, 0xd0, 0x10, 0x90, 0xb0];

const ALL_OPCODES: [u8; 151] = [
