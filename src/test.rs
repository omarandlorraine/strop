...
/// Values which a function can takes as arguments
///
/// Useful for fuzz testing a Callable
pub trait Input: std::cmp::PartialEq + Copy + std::fmt::Debug {
...
/// Values which a function can return
pub trait Output: std::cmp::PartialEq + Copy + std::fmt::Debug {
...
impl<A: Input + Copy, B: Input + Copy> Input for (A, B) {
    fn vals() -> Vec<Self> {
        let mut v = vec![];
        for a in A::vals() {
            for b in B::vals() {
...
impl<A: Output + Copy, B: Output + Copy> Output for (A, B) {
    fn error(self, other: Self) -> f64 {
        self.0.error(other.0) + self.1.error(other.1)
    }
}

impl<A: Input + Copy, B: Input + Copy, C: Input + Copy> Input for (A, B, C) {
    fn vals() -> Vec<Self> {
        let mut v = vec![];
        for a in A::vals() {
            for b in B::vals() {
...
impl<InputParameters: Input, ReturnValue: Output> TestSuite<InputParameters, ReturnValue> {
        ...
        for _ in 0..iterations {
            ...
            if let Ok(r) = target_function.call(i) {
                if (candidate).call(i) != Ok(r) {
                    // We've found another test case that didn't pass
                    self.0.push((i, r));
                    return false;
                }
            } else {
                // The function ran amok or otherwise did not return
                return false;
            }
        }
        true
