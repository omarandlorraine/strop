...
def parse_operand(operand):
    ...
    return circumfix[0] + prefix + name + postfix + circumfix[1]
...
def iprint(opcode, i):
    ...
    operands = ([f'{parse_operand(op)}' for op in i['operands']] + ['', '', ''])[:3]
    ...
    print(f'\toperands: ["{operands[0]}", "{operands[1]}", "{operands[2]}"],')
    print('}),')

print('use crate::x80::data::InstructionData;')
print('use crate::x80::data::ReadWrite;')

print('pub const UNPREFIXED: [Option<InstructionData>; 256] = [');
for (opcode, i) in op_data['unprefixed'].items():
    iprint(opcode, i);
print('];');

print('pub const CBPREFIXED: [Option<InstructionData>; 256] = [');
for (opcode, i) in op_data['cbprefixed'].items():
    iprint(opcode, i);
print('];');
